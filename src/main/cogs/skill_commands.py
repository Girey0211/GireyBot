...
from src.main.skills.loader import CREDENTIALS_DIR, GLOBAL_SKILLS_DIR
...
class SkillCreateModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        embed.add_field(
            name="파일", value=f"`src/shared/skills/{name}/SKILL.md`", inline=True,
        )
        if executor_val:
...
class SkillCommands(commands.Cog):
    ...
    async def skills_info(self, interaction: discord.Interaction, name: str):
        ...
        embed.set_footer(
            text=f"파일: src/shared/skills/{name}/SKILL.md · 본문 {len(body)}자"
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
    ...
    async def skills_delete(self, interaction: discord.Interaction, name: str):
        ...
        embed = discord.Embed(
            ...
            description=(
                f"`src/shared/skills/{name}/` 디렉토리가 완전히 삭제됩니다.\n"
                "이 작업은 되돌릴 수 없습니다."
            ),
